...
class _CandidatosTelaState extends State<CandidatosTela> {
  ...
  Future<void> _carregar() async {
    ...
    try {
      ...
      final itens = await widget.api.candidatos(page: _page, limit: 20, q: q, skill: skill);
  ...
  List<Candidato> get _filteredCandidatos {
    return _candidatos.where((candidato) {
      ...
      final matchesSkill = _filterSkill == 'all' || (candidato.skills ?? const []).map((e) => e.toLowerCase()).contains(_filterSkill.toLowerCase());
  ...
  Widget _buildSkillDropdown() {
    return DropdownButtonFormField<String>(
      ...
      items: [
        const DropdownMenuItem(value: 'all', child: Text('Todas as Habilidades')),
        ..._skills.map((s) => DropdownMenuItem(value: s, child: Text(s))).toList(),
      ],
      onChanged: (value) => setState(() => _filterSkill = value ?? 'all'),
    );
  }
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        tooltip: 'Candidatar',
                        icon: const Icon(Icons.assignment_ind_outlined, size: 18, color: TMTokens.textMuted),
                        onPressed: () => _criarCandidatura(candidato),
                      ),
                      IconButton(
                        tooltip: 'Editar',
                        icon: const Icon(Icons.edit_outlined, size: 18, color: TMTokens.textMuted),
                        onPressed: () => _abrirFormularioCandidato(editar: candidato),
                      ),
                      IconButton(
                        tooltip: 'Excluir',
                        ...
                        onPressed: () async {
                          try {
                            ...
                            await _carregar();
                            if (!mounted) return;
                            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Candidato excluído')));
                          } catch (e) {
                            if (!mounted) return;
                            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Falha ao excluir candidato')));
                          }
                        },
                      ),
                    ],
      ...
      builder: (context) => StatefulBuilder(builder: (context, setSB) {
        return AlertDialog(
            ...
            child: Column(
              ...
              children: [
                DropdownButtonFormField<String>(
                  initialValue: jobId,
                  items: [
                    ...vagas.map((v) => DropdownMenuItem(value: v['id'].toString(), child: Text(v['title']?.toString() ?? 'Vaga')))
                  ],
                  onChanged: (v) async {
                    setSB(() { jobId = v; stages = const []; stageId = null; });
                    if (v != null) {
                      try {
                        final pipe = await widget.api.obterPipeline(v);
                        final st = (pipe['stages'] as List).cast<Map<String, dynamic>>();
                        setSB(() { stages = st; stageId = st.isNotEmpty ? st.first['id'].toString() : null; });
                      } catch (_) {}
                    }
                  },
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: stages.map((s) => DropdownMenuItem(value: s['id'].toString(), child: Text(s['name'].toString()))).toList(),
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: (loading || jobId == null) ? null : () async {
                setSB(() => loading = true);
                try {
                  await widget.api.criarCandidatura(jobId: jobId!, candidateId: candidato.id, stageId: stageId);
                  if (!context.mounted) return;
                  Navigator.of(context).pop();
                  if (!context.mounted) return;
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Candidatura criada')));
                } catch (e) {
                  if (!context.mounted) return;
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Falha ao criar candidatura')));
                } finally {
  ...
  Future<void> _abrirFormularioCandidato({Candidato? editar}) async {
    ...
    final linkedin = TextEditingController(text: editar?.linkedin ?? editar?.linkedinUrl ?? '');
    final github = TextEditingController(text: editar?.github ?? editar?.githubUrl ?? '');
    final skills = TextEditingController(text: (editar?.skills ?? const []).join(', '));
    bool loading = false;
    await showDialog<void>(
      context: context,
      builder: (context) {
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 8),
                    TextField(controller: skills, decoration: const InputDecoration(labelText: 'Skills (separadas por vírgula)')),
                  ],
                ),
              ),
            ),
              ...
              ElevatedButton(
                onPressed: loading
                    ...
                    : () async {
                        ...
                        try {
                          final skillsList = skills.text.split(',').map((e) => e.trim()).where((e) => e.isNotEmpty).toList();
                          if (editar == null) {
