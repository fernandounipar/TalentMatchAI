...
async function upsertSkills(companyId, skillsArr) {
  ...
  for (const s of skillsArr) {
    ...
    const q = await db.query(
      `INSERT INTO habilidades (company_id, name)
       VALUES ($1, $2)
  ...
  try {
    ...
    let sql = `SELECT c.id, c.company_id, c.full_name, c.email, c.phone, c.linkedin, c.github_url, c.created_at,
      COALESCE((SELECT json_agg(s.name ORDER BY s.name)
                FROM habilidades_candidato cs JOIN habilidades s ON s.id = cs.skill_id
                WHERE cs.candidate_id = c.id), '[]') AS skills,
      (SELECT COUNT(*)::int FROM curriculos r WHERE r.candidate_id = c.id AND r.company_id = $1) AS qtd_curriculos,
      (SELECT COUNT(*)::int FROM entrevistas i JOIN candidaturas a ON a.id = i.application_id
        WHERE a.candidate_id = c.id AND i.company_id = $1) AS qtd_entrevistas,
      (SELECT json_agg(json_build_object('job_title', j.title, 'status', a.status, 'updated_at', a.updated_at))
       FROM candidaturas a
       JOIN vagas j ON j.id = a.job_id
       WHERE a.candidate_id = c.id AND a.company_id = $1) AS applications
      FROM candidatos c
    ...
    if (skill) {
      ...
      sql += ` AND EXISTS (
         SELECT 1 FROM habilidades_candidato cs
         JOIN habilidades s ON s.id = cs.skill_id
...
router.get('/:id', async (req, res) => {
  const r = await db.query(
    `SELECT c.*, COALESCE((SELECT json_agg(json_build_object('name', s.name, 'level', cs.level))
      FROM habilidades_candidato cs JOIN habilidades s ON s.id = cs.skill_id WHERE cs.candidate_id = c.id), '[]') AS skills
     FROM candidatos c WHERE c.id=$1 AND c.company_id=$2`,
...
router.post('/', async (req, res) => {
  try {
    ...
    if (norm.length > 0) {
      ...
      for (const s of up) {
        await db.query(
          `INSERT INTO habilidades_candidato (candidate_id, skill_id, level)
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (skills !== undefined) {
      await db.query('DELETE FROM habilidades_candidato WHERE candidate_id=$1', [cand.id]);
      const norm = normalizeSkillsInput(skills);
      if (norm.length > 0) {
        const up = await upsertSkills(req.usuario.company_id, norm);
        for (const s of up) {
...
router.delete('/:id', async (req, res) => {
  await db.query('DELETE FROM candidatos WHERE id=$1 AND company_id=$2', [req.params.id, req.usuario.company_id]);
  res.status(204).send();
});

// Obter dados do GitHub
router.get('/:id/github', async (req, res) => {
  try {
    const r = await db.query('SELECT github_url FROM candidatos WHERE id=$1 AND company_id=$2', [req.params.id, req.usuario.company_id]);
    if (!r.rows[0]) return res.status(404).json({ erro: 'Candidato não encontrado' });

    const githubUrl = r.rows[0].github_url;
    if (!githubUrl) return res.status(400).json({ erro: 'Candidato sem GitHub vinculado' });

    // Extrair username da URL (ex: https://github.com/usuario -> usuario)
    const parts = githubUrl.split('/').filter(Boolean);
    const username = parts[parts.length - 1];

    if (!username) return res.status(400).json({ erro: 'Username inválido' });

    // Fetch GitHub API (sem autenticação por enquanto, cuidado com rate limit)
    // Idealmente usar um token de app se disponível no env
    const headers = { 'User-Agent': 'TalentMatchAI' };
    ...
    if (!userRes.ok) throw new Error('Falha ao buscar usuário no GitHub');
    ...
    res.status(500).json({ erro: 'Falha ao buscar dados do GitHub' });
  }
});
