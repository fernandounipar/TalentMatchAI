...
async function aplicarMigration017() {
  const client = await pool.connect();
  
  try {
    console.log('🚀 Aplicando Migration 017: question metrics views...\n');
    
    // Ler arquivo SQL
    const sqlPath = path.join(__dirname, 'sql', '017_question_metrics_views.sql');
    ...
    const viewsToTest = [
      'question_sets_stats',
      'question_sets_by_job',
      'question_type_distribution',
      'question_sets_usage',
      'question_editing_stats'
    ];
    ...
    // Testar função get_question_set_metrics
    console.log('\n📊 Testando função get_question_set_metrics...');
    ...
    const indexes = await client.query(`
      ...
      AND indexname IN ('idx_interview_questions_origin', 'idx_question_sets_created')
      ORDER BY indexname;
    `);
    ...
    console.log('\n✅ Validação completa! Migration 017 aplicada com sucesso.');
    
  } catch (error) {
    console.error('❌ Erro ao aplicar migration 017:', error.message);
    console.error(error);
    process.exit(1);
  } finally {
    client.release();
